...
#
#$Id: page.functions.php 8539 2012-12-04 17:08:36Z calguy1000 $
  ...
  if( is_object($route) )
      ...
      else
	{
	  $matches = $route->get_results();
		...
		if ($route->checkModuleFunction()) {
			$tmp_module = $route->get_dest();
			$tmod = $gCms->GetModuleInstance($tmp_module);
			if (method_exists($tmod, 'RouteDetection')) {
				$tmatches = $tmod->RouteDetection($route);
				$matches = array_merge($matches, $tmatches);
			}
		}
	  // it's a module route
