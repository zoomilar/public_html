...
//
//
// $Id: GD.php 4169 2007-09-21 06:28:49Z silmarillion $
//
...
Class Image_Transform_Driver_GD extends Image_Transform
{
	...
	function kas($pv){
				...
				$exclu = Array('vw.png', 'header_image.png');
				
				if (in_array($pv[sizeof($pv)-1], $exclu)){
					return false;
				}else{
					return true;
				}	
    ...
    */
    function _resize($new_x, $new_y) {
		...
		if ($this->type=="png" && $this->kas($this->image)){
    ...
    function save($filename, $type = '', $quality = 100)
    {
		...
		if ($this->type=="png" && $this->kas($this->image)){
			$pav = $this->imageHandle;
			imagealphablending($pav, false);
			$color = imagecolorallocatealpha($pav, 0, 0, 0, 127);
			imagefill($pav, 0, 0, $color);
			imagesavealpha($pav, true);						
			$this->imageHandle = $pav;
		}	
