							   ...
							   $inline=false,$targetcontentonly=false,$prettyurl='')
{
	...
	if ($prettyurl != '' && $config['url_rewriting'] == 'mod_rewrite')
	{
		...
		foreach ($params as $key=>$value) {
			...
			} else if ($key == 'filter_params' && is_array($value) && count($value) > 0) {
				//unikalus
					...
					if ($kk != 'id') {
						...
						if (is_array($vv) && count($vv) > 0) {
							...
							foreach ($vv as $kk2 => $vv2) {
								$vv2 = cms_htmlentities($vv2);
								if (strpos($text, '?') === false && strpos($tmp_url, '?') === false) {							
									$tmp_url .= '?'.$value['id'].$kk.'[]='.rawurlencode($vv2);
								} else {
									$tmp_url .= '&amp;'.$value['id'].$kk.'[]='.rawurlencode($vv2);
								}
							}
							$text .= $tmp_url;
							//$vv = urlencode($tmp_url);
							//echo $vv;
						} else {
							$vv = cms_htmlentities($vv);
							if (strpos($text, '?') === false) {							
								$text .= '?'.$value['id'].$kk.'='.rawurlencode($vv);
							} else {
								$text .= '&amp;'.$value['id'].$kk.'='.rawurlencode($vv);
							}
						}
