...
abstract class ContentBase
{
  ...
  public function GetModifiedDate()
  {
 	...
 	function whatItIs($id)
	{
		global $gCms;
		$db = $gCms->GetDb();
		$query = 'SELECT type FROM '.cms_db_prefix().'content WHERE parent_id=? and type like ? LIMIT 0,1';
		$dbresult = $db->Execute($query, array($id, 'catalogitem%'));
		$result = $dbresult->FetchRow();

		$query = 'SELECT type FROM '.cms_db_prefix().'content WHERE parent_id=? and type like ? LIMIT 0,1';
		$dbresult = $db->Execute($query, array($id, 'catalogcategory%'));
		$result2 = $dbresult->FetchRow();



		$ret['itemai'] = $result['type'];
		$ret['katalogai'] = $result2['type'];
		return $ret;
	} 
   ...
   */
    ...
    */
    ...
    function LoadFromData(&$data, $loadProperties = false)
    {
     ...
     // $this->mMenuImg                   = $db->getOne("SELECT content FROM cms_content_props WHERE content_id='{$data["content_id"]}' and prop_name = 'menupav'");
