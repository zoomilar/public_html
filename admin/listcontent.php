...
function display_hierarchy(&$root, &$userid, $modifyall, &$users, &$menupos, &$openedArray, &$pagelist, &$image_true, &$image_set_false, &$image_set_true, &$upImg, &$downImg, &$viewImg, &$editImg, &$copyImg, &$deleteImg, &$expandImg, &$contractImg, &$mypages, &$page, $columnstodisplay, $author_allpages )
{
  ...
  if ($display != 'none')
	...
	if ((check_permission($userid, 'Add Pages') && count($mypages)) || check_permission($userid,'Manage All Content')){
		  $ret = $one->whatItIs($one->mId);
		
		    

		  			
			
		   if (($ret['katalogai'] && $ret['itemai'])){
			$prr = "catalogcategory";
		   }elseif (($ret['katalogai'] && !$ret['itemai'])){
			$prr = "catalogcategory";
		   }elseif (!$ret['katalogai'] && $ret['itemai']){
			$prr = "catalogitem";
		   }else{
			$prr = $one->Type();
			if ($prr == "sectionheader" || $prr == "pagelink"){$prr = "content";}
		   }
		   
		   
		  if (strstr($ret['itemai'], "catalogcategory_sp"))
			$tip = '_sp';
		  elseif (strstr($ret['itemai'], "catalogitem_sp"))
			$tip = '_sp';
		  else
			$tip='';
			...
			$prdadd .=  '&nbsp;&nbsp;<a href="addcontent.php'.$urlext.'&amp;parent_id='.$one->Id().'&amp;content_type='.$prr.'"> ';
			$prdadd .= ' <img src="themes/NCleanGrey/images/icons/system/newobject.gif" height="12px" title="'.lang('addcontent').'"/></a>';
	}else{
		unset($prdadd);
	}		
		...
		$txt .= '<a class="tooltip" href="editcontent.php'.$urlext.'&amp;content_id='.$one->Id().'&amp;page='.$page.'" title="'. cms_htmlentities($one->Name().' ('.$one->Alias().')', '', '', true). '" onmouseover="document.getElementById(\'' .$one->Id().'_info\').style.display = \'inline-block\';" onmouseout="document.getElementById(\''.$one->Id().'_info\').style.display = \'none\';">
			' . cms_htmlentities($str, '', '', true) . '<span id="'.$one->Id().'_info"><strong>'.lang('content_id').':</strong> '.$one->Id().'<br /> <strong>'.lang('title').':</strong> '. cms_htmlentities($one->Name()).'<br /> <strong>'.lang('pagealias').':</strong> '.$one->Alias().'</span></a>'.$prdadd;
	      else
