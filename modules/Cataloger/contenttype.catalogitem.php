...
#class CatalogItem extends Content
{
   ...
   function EditAsArray($adding = false, $tab = 0, $showadmin=false)
      ...
      if ($tab == 0)
         ...
         foreach ($this->attrs as $thisAttr)
            {
			//echo"<pre>";print_r($thisAttr->attr);
				//$thisAttr->attr = $module->Lang($thisAttr->alias);
				if (in_array($thisAttr->alias, array("aprasymas", "detalus")))
        ...
        $imgcount = get_site_preference('Cataloger_mapi_pref_item_image_count', '2');
         ...
         if ($imgcount != 0)
            ...
            for ($i=1; $i<= $imgcount; $i++)
               ...
               $imgsrc .= '<img width="100px" alt="'.$this->lang('nameimages').'" title="'.$this->lang('nameimages').'" src="'.
