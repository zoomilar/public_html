<?php
		...
		//if ($thispagecontent->Type() == "catalogitem_sp"){
			//echo "---".$pagecontent->Alias();
		//}
		if($params['sp']){
			$db = $gCms->GetDb();
			$cid = $db->getOne("SELECT content_id FROM ".cms_db_prefix()."content WHERE content_alias=?", array($params['alias']));
			$datos = $db->GetArray("SELECT * FROM ".cms_db_prefix()."tx_n_repertuaras WHERE spektaklioid=? ORDER BY data ASC, laikas ASC", array($cid));
			echo mysql_error();
			$this->smarty->assign_by_ref('datos',$datos);
    ...
    $tmp_T = $this->ProcessTemplateFromDatabase('catalog_' . $params['sub_template']);
    echo  $this->ProcessTemplateFromData($tmp_T);
