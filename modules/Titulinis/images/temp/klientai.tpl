...
<table cellspacing="0" class="pagetable">
	<thead>
		<tr>
			...
			<th width="200px"><div>{$Titulinis->Lang(pavadinimas)}</div></th>
			<th><div>{$Titulinis->Lang(tituliniame)}</div></th>
	...
	<tbody id='{$kalba}'>
		...
		{foreach from=$prop_array.$kl item=entry}
			...
			<tr class="row1" onmouseover="this.className='row1hover';" onmouseout="this.className='row1';" id='lt-{$nmb}'>
					...
					<td><div>{$entry->pavadinimas}</div></td>					
					<td><div>{$entry->tituliniame}</div></td>					
