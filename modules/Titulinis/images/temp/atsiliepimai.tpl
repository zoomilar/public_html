...
<table cellspacing="0" class="pagetable">
	<thead>
		<tr>
			...
			<th width="200px"><div>{$Titulinis->Lang(antraste)}</div></th>
...
{foreach from=$kalbos item=kalba key=kl}
	<tbody id='{$kalba}'>
		<tr><td colspan='7' style="background-color: #e5e5e5"><b>{*$kalba|upper*}</b></td></tr>
		...
		{foreach from=$prop_array.$kl item=entry}
			...
			<tr class="row1" onmouseover="this.className='row1hover';" onmouseout="this.className='row1';" id='lt-{$nmb}'>
					...
					<td>{if $entry->paveiksliukas}<div>{if $allow_more || ($cuser == $entry->userid)}<a href="moduleinterface.php?_sx_={$smarty.get._sx_}&mact={$mod_w},m1_,editprop,0&prop_id={$entry->id}&m1_kat={$kateg}" class="roo" id="r-{$entry->kategorija}-{$nmb}"><img style="width: 28px" src="{$root_url}/uploads/images/titulinis/{$entry->paveiksliukas}"/></a>{else}{$entry->paveiksliulas}{/if}</div>{/if}</td>	
					<td><div>{$entry->antraste}</div></td>					
