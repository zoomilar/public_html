 ...
 *
 * If the "languages_id" parameter is set, this page will
...
if (isset($params['languages_id']))
   {
   $query = 'SELECT languages_id, description, explanation from '.cms_db_prefix().
      'module_languages where languages_id = ?';
   $result = $db->Execute($query,array($params['languages_id']));

   if ($result !== false)
      {
	  // load in the record if there was no error
      $row=$result->FetchRow();
      $sid = $row['languages_id']; // stupid -- we're passing the param, and then using the database version.
...
$smarty->assign('submit', $this->CreateInputHidden($id,'languages_id',$sid).$this->CreateInputSubmit($id, 'submit', $this->Lang('submit')));
