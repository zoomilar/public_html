<link rel="stylesheet" href="../modules/News/lib/jquery.datepick.css" type="text/css"  />
<script type = "text/javascript" src = "../modules/News/lib/jquery.datepick.js" ></script>	
...
<script type="text/javascript">
 $(document).ready(function(){
	$("#dtfrom, #dtto").datepick({dateFormat: 'yyyy-mm-dd'});
 
	$("#dtfrom").change(function(){
		var th = $(this).val(), dh;
		dh = th.split("-");
		
		$("select[name='m1_postdate_Month']").val(dh[1]);
		$("select[name='m1_postdate_Day']").val(dh[2]);
		$("select[name='m1_postdate_Year']").val(dh[0]);
		
	});
	$("#dtto").change(function(){
		var th = $(this).val();
		$("#dataiki input").val(th);
	});

	$("#dtto").val($("#dataiki input").val());
...
{/if}
	...
	*}
	...
	<div class="pageoverflow">
		...
		<p class="pageinput">
			<input type="text" id="dtfrom" name="dtfrom" value="{$postdate|date_format:"%Y-%m-%d"}" style="width: 75px">
		<div style="display: none">	
			{html_select_date prefix=$postdateprefix time=$postdate start_year="-10" end_year="+15"} {html_select_time prefix=$postdateprefix time=$postdate}</p>
		</div>
	</div>
{if isset($statustext)}
	<div class="pageoverflow">
...
{foreach from=$custom_fields item='field'}
        <div class="pageoverflow">
           ...
           <p class="pageinput">
           	{if $field->prompt == "Nuotrauka"}
           	{*<img src="uploads/news/id{$articleid}/{$field->field}" alt="" />*}
            {$field->field}
            {else}
            	{$field->field}
            {/if}
