...
if (isset($params['submit']))
{
  if ($name != '')
    {
      // Make sure a category by this name doesn't already exist
      $query = 'SELECT name FROM '.cms_db_prefix().'module_products_categories WHERE name = ?';
      $value = $db->GetOne($query,array($name));
      if( $value != '' )
	{
	  echo $this->ShowErrors($this->Lang('categoryexists',$value));
	}
      else
	{
	  $query = 'INSERT INTO '.cms_db_prefix().'module_products_categories (name, create_date, modified_date) VALUES (?,?,?)';
