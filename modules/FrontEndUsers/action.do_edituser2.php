...
if( !$this->_HasSufficientPermissions('adduser') &&
    !$this->_HasSufficientPermissions('editusers') ) {
  return;
}
...
foreach( $properties as $prop ) {
  ...
  if( isset( $params['user_id'] ) ) {
    ...
    $propval = $this->GetUserPropertyFull( $prop['name'], $params['user_id'] );
  }
  ...
  switch( $defn['type'] ) {
  case 0: // text
    $onerow->control = $this->CreateInputText( $id, 'input_'.$prop['name'],
  ...
  case 4: // dropdown
    $onerow->control = $this->CreateInputDropdown($id, 'input_'.$prop['name'], 
						  $seloptions,-1, $val);
    break;

  case 5: // multiselect
    $onerow->control = $this->CreateInputSelectList($id, 'input_'.$prop['name'].'[]', 
						    $seloptions, $selected,
						    $defn['length']);
    break;
  ...
  case 6: // image
	...
	if( !$onerow->required  && isset($params['user_id']) ) {
  ...
  case 7: // radio button group
    $onerow->control = $this->CreateInputRadioGroup($id, 'input_'.$prop['name'],
						    $seloptions, $val, '', '<br/>');
    break;
