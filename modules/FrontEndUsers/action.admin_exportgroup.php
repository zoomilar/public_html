...
if( !is_array( $grp_prop_rels ) || !isset($grp_info[0]) || $grp_prop_rels[0] === FALSE )
  {
    $parms = array();
    $parms['active_tab'] = 'groups';
    $parms['error'] = $this->Lang('error_norelations');
    $this->Redirect($id,'defaultadmin',$returnid, $parms);
    return;
  }
...
foreach( $grp_prop_rels as $onerel )
{
  ...
  if( !$defn )
    {
      $parms = array();
      $parms['active_tab'] = 'groups';
      $parms['error'] = $this->Lang('error_propnotfound',$onrel['name']);
      $this->Redirect($id,'defaultadmin',$returnid, $parms);
      return;
    }
  ...
  if( $obj->type == 4 || $obj->type == 5 )
	...
	{
	  $op = new StdClass;
	  $op->name = $sel_op['option_name'];
	  $op->text = $sel_op['option_text'];
	  $ops[] = $op;
	}
      $obj->options = $ops;
    }
