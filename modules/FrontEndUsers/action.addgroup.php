...
$smarty->assign ('input_groupname', $this->CreateInputText ($id, 'input_groupname',
							    $groupname, 20, 80));
...
foreach( $propdefn as $defn ) {
  ...
  if( $sortorder > 1 ) {
    $onerow->moveup = $this->CGCreateInputSubmit($id,'moveup',$sortorder,'',
						 'icons/system/sort_up.gif');
  }
  if( $sortorder < count($propdefn) ) {
    $onerow->movedown = $this->CGCreateInputSubmit($id,'movedown',$sortorder,'',
