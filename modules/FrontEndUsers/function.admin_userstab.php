...
if( $this->_HasSufficientPermissions('listusers') ) {
  ...
  if( isset( $params['filter']) ) {
    ...
    if( isset( $params['filter_viewprops']) ) {
      ...
      $this->SetPreference('current_viewprops','');
    }
  ...
  $extraprops = '';
  $filterapplied = ($curgroup != -1 || $curregex != '' || $currloggedinonly == 1 ||
		    $viewprops != '' || $curpropsel != 'none' || $curpropval != '' );
  ...
  $offset = ($curpage - 1) * $curlimit;
  $this->GetUsersInGroup( $curgroup, $curregex, $curlimit, $cursort,
			  $curpropsel, $curpropval, $currloggedinonly, $offset );
  
  $nmatches = $this->get_lastuserquery_count();
  $users = $this->get_lastuserquery_matches();
  if( !is_array($users) ) {
    // an error occurred
    $db = $this->GetDb();
  ...
  $npages = (int)($nmatches / $curlimit);
  if( $nmatches % $curlimit != 0 ) $npages++;
  ...
  $nav['curpage'] = $curpage;
  if( $curpage > 1 ) {
    $nav['firstpage_url'] = $this->create_url($id,'defaultadmin','',
					      array('cg_activetab'=>'users','page'=>1));
    $nav['prevpage_url'] = $this->create_url($id,'defaultadmin','',
					     array('cg_activetab'=>'users','page'=>$curpage-1));
  }
  if( $curpage < $npages ) {
    $nav['nextpage_url'] = $this->create_url($id,'defaultadmin','',
					     array('cg_activetab'=>'users','page'=>$curpage+1));
    $nav['lastpage_url'] = $this->create_url($id,'defaultadmin','',
					     array('cg_activetab'=>'users','page'=>$npages));
  }
...
if( $this->_HasSufficientPermissions('adduser') ) {
  if( $this->GetPreference('require_onegroup') == 0 ||
    ...
    $smarty->assign('addlink', $this->Lang('nogroups'));
  }
 }
