...
class CGImage_Blur_Filter extends CGImageFilterBase
{
  ...
  public function transform(CGImageBase $src)
  {
    ...
    $res = imagefilter($_dest['rsrc'],IMG_FILTER_GAUSSIAN_BLUR);

    if( !$res )
      {
	throw new Exception('Error applying filter '.IMG_FILTER_GAUSSIAN_BLUR);
      }
    return $_dest;
  }
}
