...
class CGImage_MeanRemoval_Filter extends CGImageFilterBase
{
  ...
  public function transform(CGImageBase $src)
  {
    ...
    $tmp = $this->_args;
    $res = imagefilter($_dest['rsrc'],IMG_FILTER_MEAN_REMOVAL);

    if( !$res )
      {
	throw new Exception('Error applying filter '.IMG_FILTER_MEAN_REMOVAL);
      }
    return $_dest;
  }
}
