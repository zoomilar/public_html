...
class CGImage_Contrast_Filter extends CGImageFilterBase
{
  private $_contrast = 0;
  ...
  public function transform(CGImageBase $src)
  {
    ...
    $res = imagefilter($_dest['rsrc'],IMG_FILTER_CONTRAST,$this->_contrast);

    if( !$res )
      {
	throw new Exception('Error applying filter '.IMG_FILTER_CONTRAST);
      }
    return $_dest;
  }
}
