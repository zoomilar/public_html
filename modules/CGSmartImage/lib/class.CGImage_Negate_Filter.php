...
class CGImage_Negate_Filter extends CGImageFilterBase
{
  ...
  public function transform(CGImageBase $src)
  {
    ...
    $res = imagefilter($_dest['rsrc'],IMG_FILTER_NEGATE);
    imagecolortransparent($_dest['rsrc'],$tcolor);

    if( !$res )
      {
	throw new Exception('Error applying filter '.IMG_FILTER_NEGATE);
      }
    return $_dest;
  }
}
