...
class CGImage_Colorize_Filter extends CGImageFilterBase
{
  private $_args = array(0,0,0,0);
  ...
  public function __construct(/* variable arguments */)
  {
    ...
    for( $i = 0; $i < min(count($this->_args),count($args)); $i++ )
      {
	$this->_args[$i] = min(255,max(0,(int)$args[$i]));
      }
  }
  ...
  public function transform(CGImageBase $src)
  {
    ...
    $tmp = $this->_args;
    $res = imagefilter($_dest['rsrc'],IMG_FILTER_COLORIZE,$tmp[0],$tmp[1],$tmp[2],$tmp[3]);
