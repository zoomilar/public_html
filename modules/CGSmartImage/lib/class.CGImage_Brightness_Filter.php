...
class CGImage_Brightness_Filter extends CGImageFilterBase
{
  ...
  public function __construct(/* variable arguments */)
  {
    ...
    if( is_array($args) )
      {
	$this->_brightness = (int)$args[0];
      }
    else
      {
	$this->_brightness = $args;
      }
    // nothing here.
  }
  ...
  public function transform(CGImageBase $src)
  {
    ...
    $res = imagefilter($_dest['rsrc'],IMG_FILTER_BRIGHTNESS,$this->_brightness);

    if( !$res )
      {
	throw new Exception('Error applying filter '.IMG_FILTER_BRIGHTNESS);
      }
    return $_dest;
  }
}
