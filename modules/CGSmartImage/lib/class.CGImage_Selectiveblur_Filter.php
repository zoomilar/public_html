...
/**
 * Selective Blur image filter class
 *
 * Performs a selective blur on the provided image.
 * There are no parameters to this filtering method.
 *
 * @author Robert Campbell <calguy1000@cmsmadesimple.org>
 * @package CGSmartImage
 */
class CGImage_Selectiveblur_Filter extends CGImageFilterBase
{
  
  /**
   * Transform the specified image
   *
  ...
  public function transform(CGImageBase $src)
  {
    ...
    $tmp = $this->_args;
    $res = imagefilter($_dest['rsrc'],IMG_FILTER_SELECTIVE_BLUR);

    if( !$res )
      {
	throw new Exception('Error applying filter '.IMG_FILTER_SELECTIVE_BLUR);
      }
    return $_dest;
  }
}
