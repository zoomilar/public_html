 ...
 * database data.
 *
 * If the "filelists_id" parameter is -1, it's a new record.
...
if (isset($params['filelists_id']) && $params['filelists_id'] != -1)
   {
	// we received a filelists_id that was not -1, which means we're updating an
	// existing record. So we issue a SQL Update
   $query = 'UPDATE '.cms_db_prefix().
      'module_filelists set description=?, explanation=? where filelists_id = ?';
   $result = $db->Execute($query,array($params['description'],$params['explanation'],$params['filelists_id']));
   $params['module_message'] = $this->Lang('updated_record');
   }
else
   {
	// we received no filelists_id or one that was -1, which means we're creating
	// a new record. So we issue a SQL Insert. But first, we use the sequence to get a fresh ID
   $sid = $db->GenID(cms_db_prefix().'module_filelists_seq');
   $query = 'INSERT INTO '.cms_db_prefix().
      'module_filelists (filelists_id, description, explanation) VALUES (?,?,?)';
