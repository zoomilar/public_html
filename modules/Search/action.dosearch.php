...
class SearchItemCollection
{
  ...
  function AddItem($title, $url, $txt, $weight = 1, $module = '', $modulerecord = 0, $aditional_info = false)
  {
    if( $txt == '' ) { $txt = $url; }
    $exists = false;
	...
	if (!empty($module) )
	  {
		if ($module == 'Products' && $aditional_info != false) {
			
			$newitem->fields = $aditional_info;
		}
			  ...
			  if (count($searchresult) == 3)
			    {
			      $col->AddItem($searchresult[0], $searchresult[2], $searchresult[1], 
					    $result->fields['total_weight'], 
					    $modulename, $result->fields['content_id']);
			    } else if (count($searchresult) == 4) {
					$col->AddItem($searchresult[0], $searchresult[2], $searchresult[1], 
					    $result->fields['total_weight'], 
					    $modulename, $result->fields['content_id'], $searchresult[3]);
